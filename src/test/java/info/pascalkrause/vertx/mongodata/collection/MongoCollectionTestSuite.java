...
import info.pascalkrause.vertx.mongodata.service.MongoCollectionServiceImpl;
import info.pascalkrause.vertx.mongodata.service.MongoCollectionServiceVerticle;
...
public class MongoCollectionTestSuite {
    ...
    public void testServiceDatasource(TestContext testContext) throws IOException {
        Async testComplete = testContext.async();
        vertx.deployVerticle(new MongoCollectionServiceVerticle(serviceAddress, mongoConfig), deployResponse -> {
            if (deployResponse.failed()) {
                return;
            }
            MongoService mcs = new MongoCollectionServiceImpl(vertx.eventBus(), serviceAddress);
